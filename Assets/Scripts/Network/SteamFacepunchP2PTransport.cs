...
{
    public class SteamFacepunchP2PTransport : NetworkTransport
    {
        ...
        public override void Init()
        {
            ...
            {
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                    NetworkLog.LogErrorServer("SteamClient.IsValid - false - SteamClient is not Initialized, SteamP2PTransport can not run without it, init with SteamClient.Init()");
                return;
            }
            ...
            for (int i = 0; i < MLAPI_CHANNELS.Length; i++)
            {
                ...
                channelIdToName.Add(channelId, MLAPI_CHANNELS[i].Channel.ToString());
            ...
            for (int i = 0; i < UserChannels.Count; i++)
            {
                ...
                channelIdToName.Add(channelId, UserChannels[i].Channel.ToString());
                channelNameToId.Add(UserChannels[i].Channel, channelId);
            }
        }

        public override NetworkEvent PollEvent(out ulong clientId, out NetworkChannel channelName, out ArraySegment<byte> payload, out float receiveTime)
        {
            ...
            while (currentPollChannel < channelSendTypes.Count)
            {
                ...
                if (packet.HasValue)
                {
                    ...
                    else
                    {
                        ...
                        channelName = (NetworkChannel)currentPollChannel;
                        receiveTime = Time.realtimeSinceStartup;
        ...
        public override void Send(ulong clientId, ArraySegment<byte> data, NetworkChannel networkChannel)
        {
            ...
            {
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                    NetworkLog.LogErrorServer("SteamP2PTransport - Can't Send to client, channel with channelName: " + networkChannel + " is not present");
                return;
            }

            int channelId = channelNameToId[networkChannel];
        ...
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                NetworkLog.LogErrorServer("SteamP2PTransport - Shutdown");
        ...
        public override SocketTasks StartServer()
        {
            ...
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                NetworkLog.LogInfoServer("SteamP2PTransport - StartServer - ConnectToCSteamID: " + SteamClient.SteamId.ToString());
        ...
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                NetworkLog.LogInfoServer("SteamP2PTransport - OnP2PSessionRequest - m_steamIDRemote: " + userId);
        ...
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                NetworkLog.LogInfoServer("SteamP2PTransport - OnP2PSessionConnectFail - m_steamIDRemote: " + connectionAttemptFailedClientId + " Error: " + request.ToString());
