...
namespace QFSW.QC
{
    ...
    public static partial class QuantumConsoleProcessor
    {
        ...
        {
            if (!type.HasAttribute<QcIgnoreAttribute>(false))
            {
        ...
        private static bool TryAddCommand(CommandData command)
        {
            ...
            else
            {
                ...
                if (alreadyExists)
                {
                    if (loggingLevel >= LoggingLevel.Warnings)
                    {
                        ...
                        Debug.LogWarning($"Quantum Processor Warning: Could not add {fullMethodName} to the table as another method with the same alias and paramater count, {keyName}, already exists.");
                    }
        ...
        public static object InvokeCommand(string commandString)
        {
            ...
            if (!commandString.StartsWith("#define", StringComparison.CurrentCulture))
            {
                commandString = QuantumMacros.ExpandMacros(commandString);
            }
